...
import { filterByLanguage, filters } from "../utills/supportfunctions";
...
function DashboardNewSong() {
    ...
    const [audioImageCover, setAudioImageCover] = useState(null) // mp3 or any type of song URL 
    const [audioUploadingProgress, setAudioUploadingProgress] = useState(0)
    const [isAudioLoading, setIsAudioLoading] = useState(false)

    const [artistImageCover, setArtistImageCover] = useState(null);
    const [artistUploadingProecss, setArtistUploadingProecss] = useState(0);
    ...
    const [isAlbumUploaing, setIsAlbumUploaing] = useState(false)
        ...
        <div className='flex flex-col items-center justify-center p-4 border border-gray-300 gap-4 '>
            ...
            <div className="flex items-center justify-center w-60 p-4">
                {isImageLoading || isAudioLoading ? (
                    ...
                    <motion.button whileTap={{ scale: 0.75 }} className="px-8 w-full py-2 rounded-md cursor-pointer text-white bg-red-600 hover:shadow-lg" onClick={saveSong}>
                        Save Song
                    </motion.button>
                )}
            </div>
            ...
            <p className="text-xl font-semibold text-headingColor"> Artists Details</p>
            <div className="bg-card backdrop-blur-md w-full h-300 rounded-md border-2 border-dotted border-gray-300 cursor-pointer">
                {isArtistUploading && <FileLoader progress={artistUploadingProecss} />}
                {!isArtistUploading && (
                    <>
            ...
            {/* Artist Name  */}
            <input type="text" placeholder="Type your Artist's name..."
                className="w-full p-3 rounded-md text-base font-semibold text-textColor outline-none shadow-md border border-gray-300 bg-transparent"
                value={artistName}
            ...
            {/* Save Button for Artists  */}
            <div className="flex items-center justify-center w-60 p-4">
                {isArtistUploading ? (
                    <DisabledButton />
                ) : (
                    <motion.button whileTap={{ scale: 0.75 }} className="px-8 w-full py-2 rounded-md cursor-pointer text-white bg-red-600 hover:shadow-lg" onClick={saveArtist}>
                        Save Artist
                    </motion.button>
                )}
            </div>
            ...
            <div className="bg-card backdrop-blur-md w-full h-300 rounded-md border-2 border-dotted border-gray-300 cursor-pointer">
                {isAlbumUploaing && <FileLoader progress={albumUploadingProgress} />}
                {!isAlbumUploaing && (
                    <>
            ...
            {/* Album Name  */}
            <input type="text" placeholder="Type your Album's name..."
                className="w-full p-3 rounded-md text-base font-semibold text-textColor outline-none shadow-md border border-gray-300 bg-transparent"
                value={albumName}
            ...
            <div className="flex items-center justify-center w-60 p-4">
                {isAlbumUploaing ? (
                    ...
                    <motion.button whileTap={{ scale: 0.75 }} className="px-8 w-full py-2 rounded-md cursor-pointer text-white bg-red-600 hover:shadow-lg" onClick={saveAlbum}>
                        Save Album
                    </motion.button>
                )}
            </div>
...
export const FileUploader = ({ isLoading, updateState, setProgress, isImage }) => {
        ...
        <label>
            <div className="flex flex-col items-center justify-center h-full">
                <div className="flex flex-col justify-center items-center cursor-pointer">
                    ...
                    <p className="text-lg text-green-300">Click to upload {isImage ? "an image" : "an audio"} </p>
                </div>
            </div>
            <input type="file" name="upload-image" accept={`${isImage ? "image/*" : "audio/*"}`} onChange={uploadFile} className="w-0 h-0" />
        </label>
    )
}
